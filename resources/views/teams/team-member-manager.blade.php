            ...
            <x-jet-form-section submit="addTeamMember">
                ...
                <x-slot name="form">
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <x-jet-input type="email" class="mt-1 block w-full" wire:model.defer="addTeamMemberForm.email" />
                        <select
                            class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out -mt-1"
                            multiple wire:model.defer="addTeamMemberForm.email">
                            @php
                                $users = App\Models\User::all();
                            @endphp
                            @foreach ($users as $user)
                                <option class="hover:bg-gray-200" value="{{$user->email}}">{{$user->name}} ({{$user->email}})</option>
                            @endforeach
                        </select>
